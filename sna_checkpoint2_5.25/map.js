...
function initMap() {
    ...
    console.log(features_public.result.results[0].name);
    /*
    var features_public = [
        {
            name: '萬興國小地下停車場',
    ...
    */
    ...
    for (var i = 0; i < 20; i++) {
        var contentString = '<div id="content">' +
            ...
            '</div>' +
            '<div id="bodyContent">' +
            '<h1 id="firstHeading" class="firstHeading">' + features_public.result.results[0].name + '</h1>' +
            '<p>簡介：' + features_public.result.results[i].summary + '元/小時<br>' +
            '<p>收費：' + features_public.result.results[i].payex + '元/小時<br>' +
            '剩餘車位：' + features_public.result.results[i].totalCar + '個<br>' +
            '上次更新：5分鐘前</p>' +
